...
function M.new( chat, players, item, item_count, on_rolling_finished, roll_type, config, controller )
  ...
  local function on_roll( roller, roll, min, max )
    ...
    if not rolling or min ~= 1 or (max ~= tmog_threshold and max ~= os_threshold and max ~= ms_threshold and max ~= rey_threshold) then return end

    local rey_roll = max == rey_threshold
    local ms_roll = max == ms_threshold
    local os_roll = max == os_threshold
    local tmog_roll = max == tmog_threshold
    local actual_roll_type = ms_roll and RollType.MainSpec or os_roll and RollType.OffSpec  or tmog_roll and RollType.Transmog or rey_roll and RollType.Rey
